...
## 20c. The second contributor reviews the governance files; only the owner rules

**Question put.** How to resolve the deadlock recorded in 20b. Three options:
(1) add the second contributor as a second code owner on the governance paths,
as a reviewer and not an authority; (2) drop the code-owner requirement and
keep one approval from anyone with write access, which removes the per-path
protection for any later collaborator; (3) keep the bypass after all, undoing
20b. The session recommended (1).

**Owner said: "1. He becomes a reviewer on the governance files, not an
authority (my recommendation). Only you rule, and he checks."**

### What it settles

- **`.github/CODEOWNERS` names both owners on every governance path.**
  Neither can merge a change there alone: GitHub refuses self-approval, and
  20b removes the bypass.
- **His approval there means the change was checked, never that it was
  ruled.** Part 11a stands: he has no authority over governance records.
  `AGENTS.md`'s "What only the owner may change" stands. A ruling exists only
  in the owner's words.
- **A change he makes to a governance path still needs the owner's
  approval**, because the owner is the only other code owner there.

### What it does not settle

- **A review withheld.** Either owner can stall the other's change by not
  approving it. The way out is editing the ruleset, which only an organization
  Owner can do and GitHub logs. That cost was put with 20b.
