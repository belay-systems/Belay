...
(`scripts/review_due.py:102-108`, `:242`), because a review sitting on an unmerged
...
- `DUE — last review was N days ago (<file>, on <branch>).`, exit 0, followed by
  ...
  ref.` and the same first-finding line (`scripts/review_due.py:247-250`).
...
cannot answer across refs` (`scripts/review_due.py:236-238`), a non-zero exit
...
`docs/`, on every `origin/*` ref (`scripts/review_due.py:201-203`), except the body of a
review report's `## Outside text` section (`:143-187`). Nothing else — an `F-NNN` in
`CHANGELOG.md`, under `.claude/`, or in a test is invisible to it. That is still a wider view
than any other reader here has, and it is the one Belay has agreed to be bound by. If you
have not run it, run it now; if it printed SKIP, no number was allocated and there is no report to write.
